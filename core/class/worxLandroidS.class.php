  ...
  foreach (eqLogic::byType('worxLandroidS', false) as $eqpt) {
    ...
    if(config::byKey('initCloud', 'worxLandroidS') ==  true || empty($elogics) == false ){
      ...
      public static function connect_and_publish($client, $msg, $eqpt) {
        ...
        try {
          $topic = $mowerType.'/'.$macAddress.'/commandOut';
          self::$_client->setWill($MowerType."/".$macAddress."/commandIn", $msg, 0, 0);
          ...
          //self::$_client->loop();  
          self::$_client->publish($mowerType."/".$macAddress."/commandIn", $msg, 0, 0);
        ...
        public static create_mower($mowerType, $mowerName, $macAddress){
          ...
          $commandIn = $mowertype.'/'. $macAddress .'/commandIn';
