		...
		foreach (eqLogic::byType('worxLandroidS', false) as $eqpt) {
			if ($eqpt->getIsEnable() == true){
				...
				if($startTime == '00:00' or $startTime > date('H:i') or date('H:i') > $endTime) {
					...
					log::add('worxLandroidS', 'debug', 'mower sleeping : '.$eqpt->getName());
					...
					//config::save('mowingTime', '1' ,'worxLandroidS');
				}
			}
		}
	}
			...
			if($errorCode != 0 and $retryMode && $retryNr < 1 && false ){ //suppression mode retry
				...
				$eqptlist[0] = array($elogic->getConfiguration('MowerType'),$elogic->getLogicalId(),'{}');
					self::connect_and_publish($eqptlist, $client, '{}');
						...
						self::newInfo($elogic,'connexion',$json2_data->dat->modules->conn,'string',0);
						self::newInfo($elogic,'GPSLatitude',$json2_data->dat->modules->4G->GPS[0],'string',0);
						self::newInfo($elogic,'GPSLongitude',$json2_data->dat->modules->4G->GPS[1],'string',0);
